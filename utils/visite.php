...
function randomGet(){
    ...
    $conn = get_db_con();
    //根据IP和refur联合查询
    $unioQuerySql = "SELECT wxId,wxNum FROM wx_data_info where state = 0 and wxId in (SELECT wxId FROM wx_analysis_info where clientIp='".$clientIp."' and refUrl='".$refUrl."') limit 0,1";
    $unioQuery = mysqli_query($conn,$unioQuerySql);
    $unioResult = mysqli_fetch_object($unioQuery);
    if(empty($unioResult)){
      //根据ip返回
      $ipQuerySql = "SELECT wd.wxId,wd.wxNum,wd.state,wa.viewCount AS viewCount FROM wx_data_info wd INNER JOIN (SELECT wxId,viewCount FROM wx_analysis_info where clientIp='".$clientIp."') wa ON wd.wxId = wa.wxId and wd.state = 0 order by viewCount desc limit 0,1";
      $ipQuery = mysqli_query($conn,$ipQuerySql);
      $ipQueryResult = mysqli_fetch_object($ipQuery);
      if(empty($ipQueryResult)){
          //共查和ip都没有，随机返回
          $normalQuerySql = "SELECT wxId,wxNum FROM wx_data_info where state = 0";
          ...
          return array('code'=>404,'msg'=>'no data found.Add first','data'=>null);
      }
