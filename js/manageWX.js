$().ready(function(){
  ...
  $("#addWXBtn").click(function(e) {
    ...
    layer.prompt({title: '输入微信号，并确认', formType: 0}, function(wxNum, index){
      layer.close(index);
      layer.prompt({title: '复制可用的二维码图片网址，并确认', formType: 2}, function(wxQRCodeUrl, index){
        layer.close(index);
...
function addWeixinData(wxNum,wxQRCodeUrl){
      $.post('/utils/manageDao.php',{type:0,id:wxNum,url:wxQRCodeUrl},function(data){
...
function stateChange(wid,state) {
  ...
  $.post('/utils/manageDao.php',{type:1,id:wid,state:currentState},function(data){
      var jsonData = JSON.parse(data);
      if(jsonData.code == 0){
          window.location.reload();
      }else{
